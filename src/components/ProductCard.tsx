...
export default function ProductCard({ product }: { product: Product }) {
  ...
  // Lock page scroll while the full-screen view is open
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <>
      ...
      {/* Rendered via portal straight into <body> — this guarantees the
          full-screen view is never trapped inside an animated/transformed
          ancestor (a `transform` anywhere up the tree turns `position:
          fixed` into "fixed relative to that ancestor" instead of the
          real viewport, which is what caused the boxed-in bug). */}
      {mounted && createPortal(detailView, document.body)}
    </>
  );
}
